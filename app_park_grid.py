...
def visualize_optimization_result(
    ...
    for i, coord in enumerate(candidate_coords):
        if pulp.value(x.get(i)) > 0.5:
            ...
            ax.scatter(coord[0], coord[1], c="red", s=150, alpha=0.8, marker="s", edgecolors="black") # サイズを固定値に変更
            ax.text(coord[0], coord[1] - 3, f"利用者: {total_users:.0f}人", fontsize=9, ha="center", va="top", bbox=dict(facecolor='white', alpha=0.5, edgecolor='none')) # 面積表示を削除
        else:
